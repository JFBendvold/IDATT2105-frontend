...
export async function fetchUserMessageHistory(userId) {
    try {
        ...
        throw new Error("There was an error while fetching user message history: " + error.response.statusText)
    }
}
...
export async function fetchChat(requestedChat) {
    try {
        const response = await apiClient.get("/chat/" + requestedChat.chatId + "/" + requestedChat.userId) 
        return response
    } catch (error) {
